...
if total_usa > 0:
    ...
    plt.title("Internações de urgência no SUS de pacientes dos Estados Unidos, por ano", fontsize=14)
    ...
    plt.figtext(
        0.5, -0.05,
        "Nota: Dados de 2025 são parciais (até agosto de 2025) e estão indicados pela linha tracejada.",
        ha="center", fontsize=9, style="italic"
    )
    plt.subplots_adjust(bottom=0.2)
    plt.tight_layout()
    plt.savefig("internacoes_usa_por_ano.png", dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.title("Valor total dos procedimentos realizados no SUS para pacientes dos Estados Unidos, por ano (em US$)", fontsize=12)
    plt.xlabel("Ano", fontsize=12)
