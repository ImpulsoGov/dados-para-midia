...
if total_ing > 0:
    with open("analise_inglaterra.html", "w", encoding="utf-8") as f:
        ...
        internacoes_por_ano_ate_agosto = df_ing.query("MES_CMPT <= 8")
        anos_ing_ate_agosto = internacoes_por_ano_ate_agosto.groupby('ANO_CMPT').size().reset_index(name='Quantidade')
    ...
    plt.title("Internações de urgência no SUS de pacientes da Inglaterra, por ano \n (Janeiro a Agosto de cada ano)", fontsize=14)
    ...
    plt.close()
    valorespor_ano_ate_agosto = df_ing.query("MES_CMPT <= 8")
    valorespor_ano_ate_agosto = valorespor_ano_ate_agosto.groupby('ANO_CMPT')['US_TOT'].sum().reset_index()
    valorespor_ano_ate_agosto = valorespor_ano_ate_agosto.sort_values('ANO_CMPT')
    plt.figure(figsize=(10, 6))
    plt.plot(
        valorespor_ano_ate_agosto["ANO_CMPT"],
        valorespor_ano_ate_agosto["US_TOT"]
    )
    plt.title("Valor total dos procedimentos realizados no SUS para pacientes da Inglaterra, por ano  \n (Janeiro a Agosto de cada ano)", fontsize=14)
    plt.xlabel("Ano", fontsize=12)
    plt.ylabel("Valor total (US$)", fontsize=12)
    plt.xticks(valorespor_ano_ate_agosto["ANO_CMPT"], fontsize=10)
    ...
    max_y = valorespor_ano_ate_agosto["US_TOT"].max()
    plt.ylim(bottom=0, top=max_y * 1.1)
    for i, row in valorespor_ano_ate_agosto.iterrows():
        valor = row["US_TOT"]
