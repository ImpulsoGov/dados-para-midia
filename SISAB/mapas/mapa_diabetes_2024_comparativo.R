...
dados_com_urgencia_2024$diabetes_100k <- (dados_com_urgencia_2024$Diabetes / dados_com_urgencia_2024$dependentes_do_sus) * 100000
dados_sem_urgencia_2024$diabetes_100k <- (dados_sem_urgencia_2024$Diabetes / dados_sem_urgencia_2024$dependentes_do_sus) * 100000
...
min_urgencia <- min(mapa_com_urgencia$diabetes_100k, na.rm = TRUE)
max_urgencia <- max(mapa_com_urgencia$diabetes_100k, na.rm = TRUE)
min_sem_urgencia <- min(mapa_sem_urgencia$diabetes_100k, na.rm = TRUE)
max_sem_urgencia <- max(mapa_sem_urgencia$diabetes_100k, na.rm = TRUE)

# Criar mapa para atendimentos COM urgência (escala independente)
mapa1 <- ggplot(mapa_com_urgencia) +
  geom_sf(aes(fill = diabetes_100k), color = "white", size = 0.2) +
  ...
  labs(title = "Gráfico 1: Aterndimentos de diabetes com urgência") +
  theme_minimal() +
...
mapa2 <- ggplot(mapa_sem_urgencia) +
  ...
  labs(title = "Gráfico 2: Atendimentos de diabetes sem urgência") +
  theme_minimal() +
  theme(legend.position = "bottom",
        plot.title = element_text(hjust = 0.5, size = 16, face = "bold", margin = margin(t = 20, b = 10)),
        plot.subtitle = element_text(hjust = 0.5, size = 9, color = "gray50"),
...
mapa_combinado <- grid.arrange(mapa1, mapa2, ncol = 2,
                              top = textGrob("Atendimentos de diabetes em pessoas idosas que dependem\nexclusivamente do SUS por estado — 2024",
                                           gp = gpar(fontsize = 20, fontface = "bold")),
                              bottom = textGrob("Fonte: Dados SISAB e ANS",
                                              gp = gpar(fontsize = 14)))

# Salvar o mapa combinado
ggsave("../resultados/mapa_diabetes_2024_comparativo.png",
       plot = mapa_combinado,
