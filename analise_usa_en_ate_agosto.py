...
if total_usa > 0:
    with open("analise_usa_en.html", "w", encoding="utf-8") as f:
        ...
        f.write("</ul>\n")
        internacao_urgencia_ate_agosto = df_usa.query("MES_CMPT <= 8")
        anos_usa_ate_agosto = internacao_urgencia_ate_agosto.groupby('ANO_CMPT').size().reset_index(name='Quantidade')
    ...
    plt.title("Annual emergency hospital admissions of United States nationals in Brazil’s Unified Health System (SUS)\n"
    "(each year includes data through August only)", fontsize=12)
    plt.xlabel("Year", fontsize=12)
    plt.ylabel("Number of hospitalizations", fontsize=12)
    plt.xticks(anos_usa_ate_agosto["ANO_CMPT"], fontsize=10)
    ...
    plt.tight_layout()
    plt.savefig("internacoes_usa_por_ano_en_ate_agosto.png", dpi=300, bbox_inches='tight')
    plt.show()
    valores_por_ano_ate_agosto = df_usa.query("MES_CMPT <= 8")
    valores_por_ano_ate_agosto = valores_por_ano_ate_agosto.groupby('ANO_CMPT')['US_TOT'].sum().reset_index()
    ...
    plt.title("Annual total value of procedures performed in Brazil’s Unified Health System (SUS) \n for patients from the United States (each year includes data through August only)", fontsize=12)
    plt.xlabel("Year", fontsize=12)
