...
axes = axes.flatten()
all_quant = []
for pais in lista_top10:
    df_pais = df_junto[df_junto['pais'] == pais]
    ufs = df_pais.groupby('uf').size().reset_index(name='Quantidade')
    all_quant.extend(ufs['Quantidade'].tolist())
global_max = max(all_quant) if all_quant else 1
for i, pais in enumerate(lista_top10):
    ...
    if not ufs.empty:
        max_quant = ufs['Quantidade'].max()
        for _, row in ufs.iterrows():
            uf_sigla = row['uf']
            quant = row['Quantidade']
            uf_geom = gdf[gdf['sigla'] == uf_sigla]
            if not uf_geom.empty:
                centroid = uf_geom.geometry.centroid.iloc[0]
                size = (quant / global_max) * 500 if global_max > 0 else 100
                ax.scatter(centroid.x, centroid.y, s=size, color='lightgray', alpha=0.7, edgecolor='black')
    ...
    cbar.set_label('Percentual (%)', fontsize=6)
    cbar.ax.tick_params(labelsize=5)
plt.tight_layout()
fig.suptitle('Distribuição absoluta de internações por UF dos 10 países com mais internações no período de 2019 a agosto de 2025', fontsize=16, y=0.98)
plt.subplots_adjust(top=0.88, bottom=0.15, hspace=0.3)
legend_sizes = [100, 250, 500]
legend_quantities = [int(global_max * s / 500) for s in legend_sizes]
legend_labels = [str(q) for q in legend_quantities]
handles = [plt.scatter([], [], s=s, color='lightgray', edgecolor='black', alpha=0.7) for s in legend_sizes]
fig.legend(handles, legend_labels, title='Quantidade absoluta', loc='lower center', ncol=3, bbox_to_anchor=(0.5, 0.02), fontsize=8, title_fontsize=10)
fig.text(0.5, 0.01, 'Fonte: SIH/SUS - Sistema de Informações Hospitalares do SUS.', ha='center', fontsize=10)
plt.savefig('mapas_top10_absoluto_grid.png', dpi=300, bbox_inches='tight')
print("Mapas salvos em mapas_top10_absoluto_grid.png")